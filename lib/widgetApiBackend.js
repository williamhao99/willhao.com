...
// Fetch wrapper with configurable timeout and abort signal
...
// Fetch Chess.com player statistics with caching
export const fetchChessStats = async () => {
  ...
  try {
    ...
    // Calculate total games across all time controls
    const totalGames =
    ...
    // On error, return safe defaults (do not cache)
    return {
...
// Fetch Clash of Clans player data with caching
...
// Fetch current/recent Spotify track with OAuth token refresh
export const fetchSpotifyStats = async () => {
  ...
  const SPOTIFY_CACHE_TTL = 3 * 1000; // 3s cache for faster updates
  ...
  try {
    ...
    // No current track playing - check recently played
