...
export const fetchSpotifyStats = async () => {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      const errorMessage = `Spotify token error: ${tokenResponse.status}`;

      // For temporary errors (503, 502, 429), provide more context
      if (tokenResponse.status === 503) {
        throw new Error(`${errorMessage} - Spotify service temporarily unavailable`);
      } else if (tokenResponse.status === 502) {
        throw new Error(`${errorMessage} - Spotify gateway error`);
      } else if (tokenResponse.status === 429) {
        throw new Error(`${errorMessage} - Spotify rate limit exceeded`);
      } else if (tokenResponse.status === 401) {
        throw new Error(`${errorMessage} - Spotify authentication failed (check refresh token)`);
      }
